...
if (logIn) {
    ...
    document.getElementsByTagName('ion-icon')[0].onclick = () => {
        if (location.href.slice(slash) == '/index.html') {
            window.location.assign('cart/cart.html');
                }
        else if( location.href.slice(slash) == '/cart.html'){
            window.location.assign('../cart/cart.html');
        }
        else {
            window.location.assign('../../cart/cart.html');
    ...
    document.getElementsByTagName('ion-icon')[0].onclick = () => {
        ...
        else if (location.href.slice(slash) == '/signUp.html' || location.href.slice(slash) == '/login.html') {
            location.href = 'login.html';
        }
        else {
            window.location.assign('../../forms/login.html');
        }
    }
}
